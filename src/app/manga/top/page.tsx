...
const Page = () => {
	...
	const fetchData = async () => {
		setIsLoading(true);
		scroolTop();
		const mangaPopuler = await getData(
			`top/manga?limit=${15}&page=${page}${
				type !== "" ? `&type=${type}` : ""
			}${rating !== "" ? `&rating=${rating}` : ""}${
				filter !== "" ? `&filter=${filter}` : ""
			}`
		);
		setTotalPage(mangaPopuler.pagination.last_visible_page);
		setManga(mangaPopuler);
		setTotalData(mangaPopuler.pagination.items.total);
		setIsLoading(false);
	};
			...
			<Layout>
				<div className='w-full snap-x rounded-box lg:px-6 md:px-4 p-2'>
					<div className='card shadow-xl bg-gray-700'>
						...
						<div className='flex gap-2 bg-base-100  overflow-x-auto'>
							<div className='w-full text-center'>
								<DropdownType
									setPage={setPage}
						...
						{isLoading ? (
							...
							<ListPage data={manga} page="manga"/>
						)}
					</div>
				</div>

...
const Page = () => {
	...
	const fetchData = async () => {
		setIsLoading(true);
		scroolTop();
		const characterPopuler = await getData(
			`top/characters?limit=${15}&page=${page}`
		);
		setTotalPage(characterPopuler.pagination.last_visible_page);
		setCharacter(characterPopuler);
		setTotalData(characterPopuler.pagination.items.total);
		setIsLoading(false);
	};
					...
					<div className='card shadow-xl bg-gray-700'>
						<div className='grid grid-cols-1 gap-2 bg-base-100 w-full rounded-t-lg p-2'>
							<div className='w-full text-center'>
								<h5 className='lg:text-xl md:text-base text-md'>Top Character</h5>
							</div>
						</div>
						{isLoading ? (
